...
public class QueryTest {
    ...
    private void getTravelReviewUniqueRecord() {
        ...
        Query query = session.createQuery("from TravelReview where title='Venice'");
    ...
    private void getTravelReviewWithQueryParamList() {
        ...
        System.out.println("Method {getTravelReviewWithQueryParamList} results:");
        for (int i=0; i<reviews.size();i++) {
            System.out.println(reviews.get(i));
        }
        tx.commit();
    ...
    private void getTravelReviewWithSelect() {
        String SELECT_QUERY = "SELECT tr.title from TravelReview as tr";
    ...
    private void updateTravelReview(String update) {
        Session session = factory.getCurrentSession();
        Transaction tx = session.getTransaction();

        tx.begin();

        Query query = session.createQuery("update TravelReview set review=:review where id=2");
        query.setParameter("review", update);
    ...
    private void deleteTravelReview() {
        ...
        Query query = session.createQuery("delete TravelReview where id=2");
        int success = query.executeUpdate();
    ...
    public static void main(String[] args) {
        ...
        //  test.getTravelReviewUniqueRecord();
         //test.getTravelReviewWithQueryParam(JOptionPane.showInputDialog("what city:"));
        // test.getTravelReviewWithQueryParamList();
        // test.getTravelReviewWithSelect();
         //test.updateTravelReview(JOptionPane.showInputDialog("Review this city:"));
